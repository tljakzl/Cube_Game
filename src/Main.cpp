...
int main()
{
	...
	shader_for_block = new Shader(shaderPath1.u8string().c_str(), shaderPath2.u8string().c_str());
    shader = new Shader("shaders/text.vs", "shaders/text.fs");
	database = new Database(databasePath.u8string().c_str());
    ...
    projectionText = new glm::mat4(); 
    *projectionText = glm::ortho(0.0f, static_cast<float>(Core::SCR_WIDTH), 0.0f, static_cast<float>(Core::SCR_HEIGHT));
    //
    ...
    std::cout << "test pass emscripten_request_animation_frame_loop"<< std::endl;
	//addFaces.join();
	//glfwTerminate();
	return 0;
}
